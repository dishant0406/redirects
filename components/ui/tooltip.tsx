...
const Tooltip: React.FC<TooltipProps> = ({
  ...
  const [calculatedPosition, setCalculatedPosition] = useState<Position | null>(position || null);
  ...
  const calculatePosition = () => {
    ...
    const bestPosition =
      position ||
      spaces.reduce((prev, current) => (current.space > prev.space ? current : prev)).position;
        ...
        {content && (
          <div
            ...
            className={cn(
              ...
              'max-w-xs w-max',
              calculatedPosition ? positionClasses[calculatedPosition] : 'opacity-0',
              tooltipClassName
            )}
          >
            {content}
